...
{
    public class DogaXmlReader : IPoliceTransferReader
    {
        ...
        public DogaXmlReader()
        { }

        public DogaXmlReader(string path, int tvmKodu, List<BransUrun> SigortaSirketiBransList, List<Bran> branslar)
        {
        ...
        public List<Police> getPoliceler()
        {
            ...
            try
            {
                ...
                doc = new XmlDocument();
                doc.Load(filePath);
                var tanimliOdemeTipleri = _TVMService.GetListTanımliBransOdemeTipleri(tvmkodu, SigortaSirketiBirlikKodlari.SSDOGASİGORTAKOOPERATİF);
                ...
                if (s.HasChildNodes)
                {
                    ...
                    for (int i = 0; i < s.ChildNodes.Count; i++)
                    {
                        ...
                        police.GenelBilgiler.TUMBirlikKodu = SigortaSirketiBirlikKodlari.SSDOGASİGORTAKOOPERATİF;
                        ...
                        for (int indx = 0; indx < tks.Count; indx++)
                        {
                            ...
                            var tanimliBransOdemeTipleri = _TVMService.GetListTanımliBransTvmSirketOdemeTipleri(tvmkodu, SigortaSirketiBirlikKodlari.SSDOGASİGORTAKOOPERATİF, police.GenelBilgiler.BransKodu.Value);
