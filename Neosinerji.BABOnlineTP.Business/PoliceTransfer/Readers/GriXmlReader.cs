...
{
    public class GriXmlReader : IPoliceTransferReader
    {
        ...
        public GriXmlReader(string path, int tvmKodu, List<BransUrun> SigortaSirketiBransList, List<Bran> branslar)
        {
        ...
        public List<Police> getPoliceler()
        {
            ...
            try
            {
                ...
                doc = new XmlDocument();
                doc.Load(filePath);
                var tanimliOdemeTipleri = _TVMService.GetListTanımliBransOdemeTipleri(tvmkodu, SigortaSirketiBirlikKodlari.GRISIGORTA);
                ...
                if (s.HasChildNodes)
                {
                    ...
                    for (int i = 0; i < s.ChildNodes.Count; i++)
                    {
                        ...
                        police.GenelBilgiler.TUMBirlikKodu = SigortaSirketiBirlikKodlari.GRISIGORTA;
                        ...
                        for (int indx = 0; indx < tks.Count; indx++)
                        {
                            ...
                            var tanimliBransOdemeTipleri = _TVMService.GetListTanımliBransTvmSirketOdemeTipleri(tvmkodu, SigortaSirketiBirlikKodlari.GRISIGORTA, police.GenelBilgiler.BransKodu.Value);
