    ...
    if (isvalid && form3) {
        ...
        if (PrimIadeli.sigortaBilgileriKontrol()) {
            ...
            $.ajax(
                {
                    type: "POST",
                    url: "/Teklif/PrimIadeli2/Hesapla",
                    data: contents,
                    success: function (data) {
                        if (data.id > 0) {
                            teklifFiyat.kontol({ processId: data.id, guid: data.g });

                            PrimIadeli.PrimDetayPartial2Getir(data.id);
            ...
            // ==== Para Formatı Belirleniyor.
            $(".autoNumeric-custom").autoNumeric('update', { vMin: '0', vMax: '9999999', mDec: 2, aSep: ',', aDec: '.' });
...
var PrimIadeli = new function () {
    return {
        ...
        HesaplamaSecenegiChange: function () {
            ...
            var hesapSecenek = $("#GenelBilgiler_HesaplamaSecenegi").val();
            if (hesapSecenek == 1) {
                $("#vefat").hide();
                $("#GenelBilgiler_VefatTutari").addClass("ignore");
                $("#yillik").show();
                $("#span-prim-teminat").html("Vefat Teminat Tutarı");
            }
            else if (hesapSecenek == 2) {
                $("#vefat").show();
                $("#yillik").hide();
                $("#GenelBilgiler_YillikPrimTutari").addClass("ignore");
        ...
        PrimDetayPartial2Getir: function (IsDurum_id) {
            $.ajax({
                url: "/Teklif/PrimIadeli2/_DetayPartial",
                dataType: "html",
                method: "post",
