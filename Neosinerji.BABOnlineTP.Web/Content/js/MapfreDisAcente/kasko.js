$(document).ready(function () {
    ...
    $("#Teminat_AracaBagliKaravanMi_control").on("switch-change", function (e, data) {
        if (data.value) {
            $("#karavanBedel").slideDown("fast");
            $("#Teminat_AxaAracaBagliKaravanBedel").autoNumeric('init', { vMin: '0', vMax: '99999999', mDec: 0, aSep: '.', aDec: ',' });
        }
        else {
            $("#karavanBedel").slideUp("fast");
        }
    });
    $("#Teminat_KullanimGelirKaybiVarMi_control").on("switch-change", function (e, data) {
        if (data.value) {
            $("#gelirKaybiBedel").slideDown("fast");
            $("#Teminat_KullanimGelirKaybiBedel").autoNumeric('init', { vMin: '0', vMax: '99999999', mDec: 0, aSep: '.', aDec: ',' });
        }
        else {
            $("#gelirKaybiBedel").slideUp("fast");
        }
    });
    
    var sompoTeminatPlani = $("#Teminat_SompoArtiTeminatDegeri").val();
    if (sompoTeminatPlani!="") {
        $("#sompoArtiTeminatDegeri").show();
    }
    else {
        $("#sompoArtiTeminatDegeri").hide();
        $("#Teminat_SompoArtiTeminatPlani").val("");
    }
    $("#Teminat_SompoArtiTeminatPlani_control").on("switch-change", function (e, data) {
        if (data.value) {
            $("#gelirKaybiBedel").slideDown("fast");
            $("#sompoArtiTeminatDegeri").show();
        }
    ...
    $("#Arac_TescilBelgeSeriNo").change(function () {
        var tescilTarihi = $("#Arac_TrafikTescilTarihi").val();
        var saseNo = $("#Arac_SaseNo").val();
        var motorNo = $("#Arac_MotorNo").val();

        if (tescilTarihi == null || tescilTarihi == '' && saseNo == null || saseNo == '' && motorNo == null || motorNo == '') {
            ...
            GetTrafikTescilBilgi(plakaKodu, plakaNo, aracRuhsatSeriNo, aracRuhsatNo, asbisNo);
        }
    });
    ...
    $("#btn-hesapla").click(function () {
        ...
        if (isvalid) {
            ...
            var contents = $("#form1, #form2, #form3, #form4, #form5,#form6").serialize();
            ...
            $.ajax(
                {
                    ...
                    success: function (data) {
                        if (data.id > 0) {
                            ...
                            $("#KaskoDigerTeklif_TeklifId").val(data.id);
                        }
                        else if (data.id == 0) {
                            $("#btn-hesapla").button("reset");
                            alert(data.hata);
                            //$("#digerTeklifVarmi").hide();
                            //$("#btn-digerTeklifEkle").show();
                        }
                        // $("#btn-hesapla").button("reset");
                    },
    ...
    $("#btn-eski-police-sorgula").click(function () {
        ...
        if (isValid == 1) {
            ...
            $.ajax({
                dataType: "json",
                url: "/Teklif/Kasko/EskiPoliceSorgula",
    ...
    $("#Teminat_Kolonlar_control").on("switch-change", function (e, data) {
        if (data.value) {
            $(".kolon").slideDown("fast");
        }
        else {
            $(".kolon").slideUp("fast");
        }
    ...
    $("#Teminat_GroupamaYHIMSKodu").change(function () {

        var YHIMSKodu = $(this).val();

        if (YHIMSKodu == 1) {//YHİMS MADDİ / BEDENİ AYRIMLI seçilmiş ise
            $(".klasikKaskoMaddiBedeniAyrimli").show();
            $(".serbestLimit").hide();
        }
        else if (YHIMSKodu == 3) {
            $(".serbestLimit").show();
            ...
            $(".klasikKaskoMaddiBedeniAyrimli").hide();
            $(".serbestLimit").hide();
        }
    });

    $("#Teminat_GroupamaYHIMSBasamakKodu").change(function () {

        var YHIMSBasamakKodu = $(this).val();

        if (YHIMSBasamakKodu == 8) { //SERBEST LİMİT seçilmiş ise
            $(".serbestLimit").show();
        }
        else {
            $(".serbestLimit").hide();
    ...
    $("#Teminat_GroupamaManeviDahilMi_control").on("switch-change", function (e, data) {
        if (data.value) {
            $(".maneviBedel").slideDown("fast");
        }
        else {
            $(".maneviBedel").slideUp("fast");
        }
    });

    $("#KaskoDigerTeklif_DigerTeklifVarMi_control").on("switch-change", DigerTeklifKontrol);
    $(".add-digerTeklif").live("click", addDigerTeklifSatir);
    $(".remove-digerTeklif").live("click", removeDigerTeklifSatir);

    $("#btn-digerTeklifEkle").click(function () {
        KaskoDigerTeklif_TeklifId
        $("#KaskoDigerTeklif_TeklifId").attr("readonly", true);
        var contents = $("#form1, #form2, #form3, #form4, #form5,#form6").serialize();
        $("#KaskoDigerTeklif_TeklifId").attr("readonly", false);
        $.ajax(
            {
                type: "POST",
                url: "/Teklif/Kasko/DigerTeklifKaydetEkle",
                data: contents,
                success: function (data) {
                    if (data.id > 0) {
                        teklifFiyat.kontol({ processId: data.id, guid: data.g });
...
$("#Arac_AnadoluKullanimTip").change(function () {
    ...
    if ((KullanimTarzi != null && KullanimTarzi != '') && (Model != null && Model != '') && (MarkaKodu != null && MarkaKodu != '') && (KullanimTipi != null && KullanimTipi != '')) {
        //$.getJSON('/Teklif/Kasko/KullanimSekliSorgulaAnadolu', { AnadoluMarkaKodu: AnadoluMarkaKodu, AracKullanimTarzi: KullanimTarzi, AracModelYili: Model, AracMarkaKodu: MarkaKodu, KullanimTipi: KullanimTipi },
...
function setPlakaSorgu(data) {
    ...
    if (data) {
        ...
        if (data.TramerBelgeNumarasi != "" && data.TramerBelgeNumarasi != null) {
            $("#Arac_TramerBelgeNumarasi").val(data.TramerBelgeNumarasi);
        }
        else {
            $("#Arac_TramerBelgeNumarasi").val("");
        }
        if (data.TramerBelgeTarihi != "" && data.TramerBelgeTarihi != null) {
            $("#Arac_TramerBelgeTarihi").val(data.TramerBelgeTarihi);
        }
        else {
            $("#Arac_TramerBelgeTarihi").val("");
        }
        ...
        $("#Teminat_TasinanYukKademe").dropDownFill(data.YurticiKademeler);
        ...
        if (data.TrafigeCikisTarihi != null && data.TrafigeCikisTarihi.length > 0) {
            $('#Arac_TrafigeCikisTarihi').datepicker('setDate', data.TrafigeCikisTarihi);
        } else {
        ...
        if (data.EskiPoliceNo != null && data.EskiPoliceNo.length > 0 && data.EskiPoliceNo != "0" && data.EskiPoliceNo != "") {
            ...
            if (data.YeniPoliceBaslangicTarih != null && data.YeniPoliceBaslangicTarih != "") {
    ...
    return {
        ...
        kredikartiOdeme: function () {
            ...
            if (teklifId && fiyat) {
                ...
                var blokeliKart = $("#OdemeTipi_6").is(':checked');
                if (nakit) {
...
function GetTrafikTescilBilgi(plakaKodu, plakaNo, aracRuhsatSeriNo, aracRuhsatNo, asbisNo) {

    if (aracRuhsatNo > 0 || asbisNo > 0) {
        $.ajax({
            dataType: "json",
            url: "/Teklif/Kasko/GetTrafikTescilBilgi",
            data: {
                PlakaKodu: plakaKodu,
                PlakaNo: plakaNo,
                AracRuhsatSeriNo: aracRuhsatSeriNo,
            ...
            success: function (data) {
                ...
                $('#Arac_TrafikTescilTarihi').datepicker('setDate', data.AracTrafikTescilTarihi);
                $('#Arac_TrafigeCikisTarihi').datepicker('setDate', data.AracTrafikTescilTarihi);
                $("#Arac_MotorNo").val(data.MotorNo);
                $("#Arac_SaseNo").val(data.SasiNo);
...
function GetGroupamaOzelAlanlar() {
    ...
    for (var i = 0; i < divSayisi; i++) {
        ...
        var tumValue = $("#TeklifUM_" + i + "__TUMKodu").val();
        if (tumValue == 26) {
            var Groupama = $("#TeklifUM_" + i + "__TeklifAl_control").bootstrapSwitch('status');
            if (Groupama) {
                $("#GroupamaOzel").show();
            }
            else {
...
function DigerTeklifKontrol(e, data) {
    if (data.value) {
        $("#teklifler").slideDown("fast");
        if (!digerTeklifSatirEkleme)
            addDigerTeklifSatir();
        digerTeklifSatirEkleme = false;
    }
    else {
        $("#teklifler").slideUp("fast");
        removeDigerTeklifSatirlar();
    }
}
function addDigerTeklifSatir() {
    _.templateSettings.variable = "rc";
    var template = _.template(
        $("script.template-teklif").html()
    );
    var templateData = { no: digerTeklifSatir };
    var elements = $("#teklif-header").parent().append(template(templateData));
    $("#KaskoDigerTeklif_DigerTeklifler_" + digerTeklifSatir + "__SirketKod").rules('add', 'required');
    //$("#KaskoDigerTeklif_DigerTeklifler_" + digerTeklifSatir + "__SirketTeklifNo").rules('add', 'required');
    //$("#KaskoDigerTeklif_DigerTeklifler_" + digerTeklifSatir + "__HasarsizlikIndirimSurprim").autoNumeric('init', { vMin: '0', vMax: '9999999', mDec: 0, aSep: '', aDec: ',' });
    $("#KaskoDigerTeklif_DigerTeklifler_" + digerTeklifSatir + "__TeklifTutar").autoNumeric('init', { vMin: '0', vMax: '9999999', mDec: 0, aSep: '', aDec: ',' });
    digerTeklifSatir++;
}

function removeDigerTeklifSatirlar() {
    digerTeklifSatir = 0;
    $("#teklif-header").siblings().remove();
}
function removeDigerTeklifSatir() {
    var id = $(this).data("id");
    var row = "#DigerTeklif-row-" + id;
    $("#teklif-header").siblings(row).remove();
}